...
local medusaResetEnabled = false -- unused, kept for config compat
...
local autoSwitchSpeedEnabled=false -- unused
...
local ragdollGuiEnabled=false  -- removed from GUI
...
local startAutoSteal,stopAutoSteal,enableAntiKick,disableAntiKick,toggleCarryMode,toggleLaggerMode,toggleLaggerCarryMode
...
local function onHumanoidStateChanged(old,new)
    ...
    if isRag and not hum.PlatformStand and not activeBatBillboard then
        activeBatBillboard=createRagdollBillboard(2.6,"RAGDOLL",Color3.fromRGB(160,80,255))
...
local function setupSpeedIndicator(char)
    ...
    local discordLabel=Instance.new("TextLabel",bb);discordLabel.Size=UDim2.new(1,0,0.4,0);discordLabel.BackgroundTransparency=1;discordLabel.Text=".gg/silence"
    discordLabel.TextColor3=Color3.fromRGB(200,170,255);discordLabel.Font=Enum.Font.GothamBold;discordLabel.TextScaled=true;discordLabel.TextStrokeTransparency=0
    ...
    local gr1=addShimmerToLabel(speedLabel,Color3.fromRGB(160,80,255),Color3.fromRGB(255,255,255))
    local gr2=addShimmerToLabel(discordLabel,Color3.fromRGB(190,160,255),Color3.fromRGB(255,255,255))
...
local KB={DropBrainrot={kb=nil,gp=nil},AutoLeft={kb=nil,gp=nil},AutoRight={kb=nil,gp=nil},AutoBat={kb=nil,gp=nil},AimbotV2={kb=nil,gp=nil},TPFloor={kb=nil,gp=nil},InstaReset={kb=nil,gp=nil},GuiHide={kb=nil,gp=nil},SpeedToggle={kb=nil,gp=nil},LaggerToggle={kb=nil,gp=nil},AntiDesync={kb=nil,gp=nil},LaggerPanel={kb=nil,gp=nil}}
    ...
    aimbotConn=RunService.RenderStepped:Connect(function()
        ...
        pcall(function()
            if not _predBall then
                ...
                _predBall.Color=Color3.fromRGB(160,0,255)
...
startAimbotV2=function()
    ...
    aimbotV2Conn=RunService.Heartbeat:Connect(function()
        ...
        if target and target.Character then
            ...
            if tr then
                ...
                local direction=(aimPoint-root2.Position).Unit
                root2.Velocity=direction*AIMBOT_SPD
                if dist<=VYSE_HIT_DIST and autoSwingEnabled then swingCurrentBat() end
            end
        else
            root2.Velocity=Vector3.new(0,root2.Velocity.Y,0)
        end
    end)
...
saveConfig=function()
    ...
    local cfg={normalSpeed=NS,carrySpeed=CS,dropBrainrotKey=ks(KB.DropBrainrot),autoLeftKey=ks(KB.AutoLeft),autoRightKey=ks(KB.AutoRight),autoBatKey=ks(KB.AutoBat),aimbotV2Key=ks(KB.AimbotV2),laggerToggleKey=ks(KB.LaggerToggle),tpFloorKey=ks(KB.TPFloor),instaResetKey=ks(KB.InstaReset),guiHideKey=ks(KB.GuiHide),speedToggleKey=ks(KB.SpeedToggle),antiDesyncKey=ks(KB.AntiDesync),grabRadius=Steal.StealRadius,stealDuration=Steal.StealDuration,antiRagdoll=antiRagdollEnabled,autoStealEnabled=Steal.AutoStealEnabled,infiniteJump=infJumpEnabled,infJumpMode=infJumpMode,medusaCounter=medusaCounterEnabled,batCounter=batCounterEnabled,carrySpeedActive=carrySpeedActive,laggerModeEnabled=laggerModeEnabled,laggerCarryActive=laggerCarryActive,laggerSpeed=LAGGER_SPEED,laggerCarrySpeed=LAGGER_CARRY_SPEED,autoBat=autoBatEnabled,aimbotV2=aimbotV2Enabled,autoSwing=autoSwingEnabled,unwalkEnabled=unwalkEnabled,antiLag=antiLagEnabled,stretchRez=stretchRezEnabled,autoTPEnabled=autoTPEnabled,autoTPHeight=autoTPHeight,guiTransparencyEnabled=guiTransparencyEnabled,mobileButtonsEnabled=mobileButtonsEnabled,mobileButtonsLocked=mobileButtonsLocked,mobileButtonsSize=mobileButtonsSize,circleButtonsEnabled=circleButtonsEnabled,antiKick=antiKickEnabled,fovValue=fovValue,perButtonDrag=perButtonDragEnabled,skyTheme=currentSkyTheme,medusaReset=nil,autoMoveSwing=autoMoveSwingEnabled,autoMoveSwingInterval=autoMoveSwingInterval,ragdollGui=ragdollGuiEnabled}
    if writefile then pcall(function() writefile("silencedvs.json",HS:JSONEncode(cfg)) end) end
end
...
toggleLaggerMode=function()
    if laggerModeEnabled then
        -- If Lagger Mode is on, turn it off and switch to Lagger Carry
        laggerModeEnabled = false
        laggerCarryActive = true
    else
        -- If Lagger Mode is off, turn it on and turn off Lagger Carry
...
toggleLaggerCarryMode=function()
    if laggerCarryActive then
        -- If Lagger Carry is on, turn it off and switch to Lagger Mode
        laggerCarryActive = false
        laggerModeEnabled = true
    else
        -- If Lagger Carry is off, turn it on and turn off Lagger Mode
        laggerCarryActive = true
        laggerModeEnabled = false
    end
    refreshSpeedModeLabel()
...
local function createStealBar()
    ...
    local PINK=Color3.fromRGB(160,80,255)
    local WHITE=Color3.fromRGB(255,255,255)
    ...
    -- silence.vs styled steal bar
    local SB_BG    = Color3.fromRGB(8,5,16)
    local SB_PINK  = Color3.fromRGB(160,80,255)
    local SB_PINK2 = Color3.fromRGB(200,130,255)
    local SB_WHITE = Color3.fromRGB(255,255,255)
    local SB_GRAY  = Color3.fromRGB(160,140,210)
    ...
    stealBarFrame.BackgroundColor3=SB_BG;stealBarFrame.BorderSizePixel=0;stealBarFrame.ZIndex=20;stealBarFrame.ClipsDescendants=true
    Instance.new("UICorner",stealBarFrame).CornerRadius=UDim.new(1,0)
    local sbStroke=Instance.new("UIStroke",stealBarFrame);sbStroke.Color=SB_PINK;sbStroke.Thickness=2;sbStroke.Transparency=0.1
    task.spawn(function()
        local t=0
        while sbStroke and sbStroke.Parent do
            t=t+0.05
            sbStroke.Transparency=0.05+math.abs(math.sin(t*2.5))*0.25
            sbStroke.Color=Color3.fromRGB(
                math.clamp(math.floor(160+math.abs(math.sin(t*1.5))*60),0,255),
                math.clamp(math.floor(80+math.abs(math.sin(t*1.5))*40),0,255),
                255)
            task.wait(0.04)
        end
    end)
    -- Fill bar with shimmer gradient
    local fillLine=Instance.new("Frame",stealBarFrame);fillLine.Size=UDim2.new(0,0,1,0)
    fillLine.BackgroundColor3=SB_PINK;fillLine.BorderSizePixel=0;fillLine.ZIndex=21
    Instance.new("UICorner",fillLine).CornerRadius=UDim.new(1,0)
    local fillGrad=Instance.new("UIGradient",fillLine)
    fillGrad.Color=ColorSequence.new({
        ColorSequenceKeypoint.new(0,Color3.fromRGB(130,60,220)),
        ColorSequenceKeypoint.new(0.5,Color3.fromRGB(200,130,255)),
        ColorSequenceKeypoint.new(1,Color3.fromRGB(130,60,220))
    })
    task.spawn(function() local t=0;while fillGrad and fillGrad.Parent do t=t+0.04;fillGrad.Offset=Vector2.new(math.sin(t)*0.5,0);task.wait(0.04) end end)
    -- Left section: STEAL label + pct
    local stealSection=Instance.new("Frame",stealBarFrame)
    stealSection.Size=UDim2.new(0,110,1,0);stealSection.Position=UDim2.new(0,14,0,0)
    stealSection.BackgroundTransparency=1;stealSection.ZIndex=25
    local stealLbl=Instance.new("TextLabel",stealSection)
    stealLbl.Size=UDim2.new(0,52,1,0);stealLbl.Position=UDim2.new(0,0,0,0)
    stealLbl.BackgroundTransparency=1;stealLbl.Text="STEAL"
    stealLbl.TextColor3=SB_WHITE;stealLbl.Font=Enum.Font.GothamBlack;stealLbl.TextSize=11
    stealLbl.TextXAlignment=Enum.TextXAlignment.Left;stealLbl.ZIndex=26
    -- shimmer on STEAL label
    local stealGrad=Instance.new("UIGradient",stealLbl)
    stealGrad.Color=ColorSequence.new({ColorSequenceKeypoint.new(0,SB_PINK),ColorSequenceKeypoint.new(0.5,SB_WHITE),ColorSequenceKeypoint.new(1,SB_PINK)})
    stealGrad.Transparency=NumberSequence.new({NumberSequenceKeypoint.new(0,0.3,0),NumberSequenceKeypoint.new(0.5,0,0),NumberSequenceKeypoint.new(1,0.3,0)})
    task.spawn(function() local t=0;while stealGrad and stealGrad.Parent do t=t+0.04;stealGrad.Offset=Vector2.new(math.sin(t)*0.5,0);task.wait(0.04) end end)
    local pctLbl=Instance.new("TextLabel",stealSection)
    pctLbl.Size=UDim2.new(0,48,1,0);pctLbl.Position=UDim2.new(0,54,0,0)
    pctLbl.BackgroundTransparency=1;pctLbl.Text="0%"
    pctLbl.TextColor3=SB_PINK2;pctLbl.Font=Enum.Font.GothamBlack;pctLbl.TextSize=11
    ...
    div1.BackgroundColor3=SB_PINK;div1.BackgroundTransparency=0.55;div1.BorderSizePixel=0;div1.ZIndex=25
    -- Right section: FPS
    ...
    fpsLbl.TextColor3=SB_GRAY;fpsLbl.Font=Enum.Font.GothamBold;fpsLbl.TextSize=10
...
local function buildMobileButtons()
    ...
    local buttons={
        ...
        {key="aimbotV2",    label="AIMBOT\nV2",     toggle=true,  exclusive=true},
    }
    ...
    local PINK_ON      = Color3.fromRGB(160,80,255)
    local BTN_BG_OFF   = Color3.fromRGB(18,12,36)       -- dark maroon card
    local PANEL_BG     = Color3.fromRGB(10,6,18)        -- even darker panel behind buttons
    local BORDER_OFF   = Color3.fromRGB(100,50,200)      -- dim pink border when off
    local BORDER_ON    = Color3.fromRGB(160,80,255)     -- bright pink border when on
    local TEXT_OFF     = Color3.fromRGB(210,185,255)     -- soft pink-white text off
    ...
    panelStroke.Color=Color3.fromRGB(80,40,160);panelStroke.Thickness=2;panelStroke.Transparency=0.3
    ...
    for i,def in ipairs(buttons) do
        ...
        btn.Activated:Connect(function()
            ...
            if def.exclusive then
                local alreadyOn=(def.key=="autoLeft" and autoLeftEnabled) or (def.key=="autoRight" and autoRightEnabled) or (def.key=="autoBat" and autoBatEnabled) or (def.key=="aimbotV2" and aimbotV2Enabled)
                if alreadyOn then
                    ...
                    elseif def.key=="aimbotV2" then stopAimbotV2();setOn(false) end
                    return
                end
                ...
                elseif def.key=="aimbotV2" then startAimbotV2();setOn(true) end
                return
            end
...
local function buildGui()
    ...
    local BG         = Color3.fromRGB(8, 6, 12)      -- main background (very dark maroon)
    local BG2        = Color3.fromRGB(12, 8, 20)      -- header background
    local ROW_BG     = Color3.fromRGB(14, 10, 22)      -- row background
    local ROW_BORDER = Color3.fromRGB(50, 30, 90)      -- row border
    local WHITE      = Color3.fromRGB(255, 255, 255)
    local GRAY       = Color3.fromRGB(160, 140, 210)
    local INP        = Color3.fromRGB(8, 5, 16)
    local PINK       = Color3.fromRGB(160, 80, 255)
    local PINK2      = Color3.fromRGB(130, 60, 220)
    local PINK3      = Color3.fromRGB(70, 30, 140)
    local OFF        = Color3.fromRGB(20, 14, 38)
    local TAB_ACTIVE = Color3.fromRGB(160, 80, 255)
    local TAB_INACT  = Color3.fromRGB(140, 120, 200)
    local SECT_LBL   = Color3.fromRGB(180, 150, 255)   -- section label color
    ...
    task.spawn(function()
        ...
        while mfStroke and mfStroke.Parent do
            ...
            mfStroke.Color=Color3.fromRGB(math.floor(160+math.sin(t*2)*30),math.floor(80+math.sin(t*2)*20),255)
            task.wait(0.04)
        end
    end)

    local bgImage=Instance.new("ImageLabel",mainFrame);bgImage.Size=UDim2.new(1,0,1,0);bgImage.BackgroundTransparency=1
    bgImage.Image="rbxassetid://138154962411611";bgImage.ScaleType=Enum.ScaleType.Crop;bgImage.ZIndex=6;bgImage.ImageTransparency=0.1;bgImage.Visible=true
    Instance.new("UICorner",bgImage).CornerRadius=UDim.new(0,18)
    local bgWatermark=Instance.new("TextLabel",bgImage)
    bgWatermark.Size=UDim2.new(1,0,1,0);bgWatermark.BackgroundTransparency=1
    bgWatermark.Text="silence.vs";bgWatermark.TextColor3=Color3.fromRGB(255,255,255)
    bgWatermark.Font=Enum.Font.GothamBlack;bgWatermark.TextSize=38
    bgWatermark.TextTransparency=0.4;bgWatermark.ZIndex=7
    bgWatermark.TextXAlignment=Enum.TextXAlignment.Center;bgWatermark.TextYAlignment=Enum.TextYAlignment.Center
    ...
    local function applyTransparencyToGui(transparent)
        if not transparent then
            ...
            bgImage.ImageTransparency=0.1;bgImage.Visible=true
            return
        end
    ...
    titleLbl.BackgroundTransparency=1;titleLbl.Text="Silenced.vs"
    ...
    discSmall.BackgroundTransparency=1;discSmall.Text="discord.gg/silencevs"
    ...
    miniBtn.Size=UDim2.new(0,110,0,28);miniBtn.Position=UDim2.new(0.5,-55,0,60)
    miniBtn.BackgroundColor3=BG2;miniBtn.BorderSizePixel=0
    miniBtn.Text="Silenced.vs";miniBtn.TextColor3=PINK
    ...
    local TABS={"Speed","Combat","Steal","Visual","Settings"}
    ...
    for _,name in ipairs(TABS) do
        ...
        lbl.TextSize=9;lbl.AutoButtonColor=false;lbl.ZIndex=8
    ...
    do
        local row=mkRow(pg,30);mkLabel(row,"Mode Toggle")
        ...
        pill.BackgroundColor3=OFF;pill.BorderSizePixel=0;pill.ZIndex=9
        Instance.new("UICorner",pill).CornerRadius=UDim.new(1,0)
        dot.Parent=pill;dot.Size=UDim2.new(0,10,0,10);dot.Position=UDim2.new(0,3,0.5,-5)
        dot.BackgroundColor3=GRAY;dot.BorderSizePixel=0;dot.ZIndex=10
        Instance.new("UICorner",dot).CornerRadius=UDim.new(1,0)
    ...
    mkBoxRow(pg,"Lagger Carry Speed",LAGGER_CARRY_SPEED,function(v) if v>0 and v<=500 then LAGGER_CARRY_SPEED=v end;saveConfig() end)

    -- Improved Lagger Mode Toggle with enhanced visual feedback
    do
        local row=mkRow(pg,30);mkLabel(row,"Lagger Status")
        ...
        -- Status text label showing ON/OFF with color coding
        local statusLbl=Instance.new("TextLabel",row)
        statusLbl.Size=UDim2.new(0,40,0,14);statusLbl.Position=UDim2.new(0,10,0.5,-7)
        statusLbl.BackgroundTransparency=1;statusLbl.Text=laggerModeEnabled and "ON" or "OFF"
        statusLbl.TextColor3=laggerModeEnabled and Color3.fromRGB(100,255,130) or Color3.fromRGB(255,90,90)
        statusLbl.Font=Enum.Font.GothamBold;statusLbl.TextSize=11;statusLbl.ZIndex=9
        
        -- Toggle button with improved feedback
        local clk=Instance.new("TextButton",pill);clk.Size=UDim2.new(1,0,1,0);clk.BackgroundTransparency=1;clk.Text="";clk.ZIndex=11
        clk.Activated:Connect(function() 
            toggleLaggerMode()
            statusLbl.Text=laggerModeEnabled and "ON" or "OFF"
            statusLbl.TextColor3=laggerModeEnabled and Color3.fromRGB(100,255,130) or Color3.fromRGB(255,90,90)
    ...
    do
        local sv=mkToggleKB(pg,"Auto Bat Aimbot",KB.AutoBat,function(on)
            ...
            if on then
                ...
                if aimbotV2Enabled then stopAimbotV2();if aimbotV2SetVisual then aimbotV2SetVisual(false) end;if mobBtnRefs.aimbotV2 then mobBtnRefs.aimbotV2(false) end end
    ...
    do
        local sv=mkToggleKB(pg,"Aimbot V2",KB.AimbotV2,function(on)
            if _anyKeyListening then return end
            if on then
                if autoLeftEnabled then autoLeftEnabled=false;stopAutoLeft();if autoLeftSetVisual then autoLeftSetVisual(false) end;if mobBtnRefs.autoLeft then mobBtnRefs.autoLeft(false) end end
                if autoRightEnabled then autoRightEnabled=false;stopAutoRight();if autoRightSetVisual then autoRightSetVisual(false) end;if mobBtnRefs.autoRight then mobBtnRefs.autoRight(false) end end
                if autoBatEnabled then stopBatAimbot();if autoBatSetVisual then autoBatSetVisual(false) end;if mobBtnRefs.autoBat then mobBtnRefs.autoBat(false) end end
                startAimbotV2();if mobBtnRefs.aimbotV2 then mobBtnRefs.aimbotV2(true) end
            else stopAimbotV2();if mobBtnRefs.aimbotV2 then mobBtnRefs.aimbotV2(false) end end
            saveConfig()
        end,function() saveConfig() end)
        aimbotV2SetVisual=sv
    end
    do
    ...
    mkSectionLbl(pg,"Panels")
    do
        local row=mkRow(pg,30);mkLabel(row,"Lagger Panel")
        local openBtn=Instance.new("TextButton",row)
        openBtn.Size=UDim2.new(0,50,0,18);openBtn.Position=UDim2.new(1,-58,0.5,-9)
        openBtn.BackgroundColor3=PINK3;openBtn.BorderSizePixel=0
        openBtn.Text="Open";openBtn.TextColor3=WHITE
        openBtn.Font=Enum.Font.GothamBold;openBtn.TextSize=9;openBtn.ZIndex=9
        openBtn.AutoButtonColor=false
        Instance.new("UICorner",openBtn).CornerRadius=UDim.new(0,5)
        local _duelPanelGui=nil
        openBtn.Activated:Connect(function()
            -- If already open, just show/destroy toggle
            if _duelPanelGui and _duelPanelGui.Parent then
                _duelPanelGui:Destroy();_duelPanelGui=nil
                _G._SilenceLaggerPanelFlip=nil;_G._SilenceLaggerPanelGui=nil
                openBtn.Text="Open"
                TweenService:Create(openBtn,TweenInfo.new(0.1),{BackgroundColor3=PINK3}):Play()
                return
            end
            TweenService:Create(openBtn,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(100,50,200)}):Play()
            task.delay(0.15,function() TweenService:Create(openBtn,TweenInfo.new(0.1),{BackgroundColor3=PINK3}):Play() end)
            openBtn.Text="Close"
            -- ── Duel Panel (IrishLagger embedded) ──────────────────────
            local UIS2=game:GetService("UserInputService")
            local Players2=game:GetService("Players")
            local plr2=Players2.LocalPlayer
            local plrGui2=plr2:WaitForChild("PlayerGui")
            local cfg2={x=2.5,y=1,z=0.15}
            local remoteStuff2="RobloxReplicatedStorage.SetPlayerBlockList"
            local textThing2,frameThing2,btnThing2,guiStatusLabel2,logoThing2
            local active2=false
            local hotkey2=nil
            -- cleanup duplicates
            for _,kid in pairs(plrGui2:GetChildren()) do if kid.Name=="IrishLaggerGui" then kid:Destroy() end end
            local screenThing2=Instance.new("ScreenGui")
            screenThing2.Name="IrishLaggerGui";screenThing2.ResetOnSpawn=false;screenThing2.IgnoreGuiInset=true
            pcall(function() if syn and syn.protect_gui then syn.protect_gui(screenThing2) end end)
            if not pcall(function() screenThing2.Parent=game:GetService("CoreGui") end) then screenThing2.Parent=plrGui2 end
            _duelPanelGui=screenThing2
            -- Main Frame (wider, less tall)
            frameThing2=Instance.new("Frame");frameThing2.Name="MainFrame"
            frameThing2.Size=UDim2.new(0,280,0,140);frameThing2.Position=UDim2.new(0.5,-140,0.5,-70)
            frameThing2.BackgroundColor3=Color3.fromRGB(8,8,12);frameThing2.BackgroundTransparency=0.08
            frameThing2.BorderSizePixel=0;frameThing2.Active=true;frameThing2.Draggable=true
            frameThing2.ClipsDescendants=false;frameThing2.Parent=screenThing2
            local uiScale2=Instance.new("UIScale");uiScale2.Scale=1;uiScale2.Parent=frameThing2
            local function updateScale2()
                local vs=workspace.CurrentCamera.ViewportSize
                uiScale2.Scale=math.clamp(vs.Y/850,0.65,1.35)
            end
            updateScale2()
            workspace.CurrentCamera:GetPropertyChangedSignal("ViewportSize"):Connect(updateScale2)
            Instance.new("UICorner",frameThing2).CornerRadius=UDim.new(0,14)
            local borderThing2=Instance.new("UIStroke");borderThing2.Color=Color3.fromRGB(90,50,140)
            borderThing2.Thickness=2;borderThing2.Transparency=0.25
            borderThing2.ApplyStrokeMode=Enum.ApplyStrokeMode.Border;borderThing2.Parent=frameThing2
            -- Header bar
            local hdr2=Instance.new("Frame");hdr2.Size=UDim2.new(1,0,0,44)
            hdr2.BackgroundColor3=Color3.fromRGB(12,8,20);hdr2.BorderSizePixel=0;hdr2.ZIndex=6;hdr2.Parent=frameThing2
            Instance.new("UICorner",hdr2).CornerRadius=UDim.new(0,14)
            local hdrPatch2=Instance.new("Frame",hdr2);hdrPatch2.Size=UDim2.new(1,0,0,14)
            hdrPatch2.Position=UDim2.new(0,0,1,-14);hdrPatch2.BackgroundColor3=Color3.fromRGB(12,8,20);hdrPatch2.BorderSizePixel=0;hdrPatch2.ZIndex=5
            local hdrDiv2=Instance.new("Frame",hdr2);hdrDiv2.Size=UDim2.new(1,0,0,1)
            hdrDiv2.Position=UDim2.new(0,0,1,-1);hdrDiv2.BackgroundColor3=Color3.fromRGB(50,30,90);hdrDiv2.BorderSizePixel=0;hdrDiv2.ZIndex=7
            -- Title: "Silence" white + ".vs" pink
            local silenceLbl=Instance.new("TextLabel",hdr2)
            silenceLbl.Size=UDim2.new(0,70,1,0);silenceLbl.Position=UDim2.new(0,14,0,0)
            silenceLbl.BackgroundTransparency=1;silenceLbl.Text="Silence"
            silenceLbl.TextColor3=Color3.fromRGB(255,255,255);silenceLbl.Font=Enum.Font.GothamBlack
            silenceLbl.TextSize=15;silenceLbl.TextXAlignment=Enum.TextXAlignment.Left;silenceLbl.ZIndex=8
            local vsLbl2=Instance.new("TextLabel",hdr2)
            vsLbl2.Size=UDim2.new(0,30,1,0);vsLbl2.Position=UDim2.new(0,84,0,2)
            vsLbl2.BackgroundTransparency=1;vsLbl2.Text=".vs"
            vsLbl2.TextColor3=Color3.fromRGB(160,80,255);vsLbl2.Font=Enum.Font.GothamBold
            vsLbl2.TextSize=11;vsLbl2.TextXAlignment=Enum.TextXAlignment.Left;vsLbl2.ZIndex=8
            -- Sub-label: "lagger panel"
            local subLbl2=Instance.new("TextLabel",hdr2)
            subLbl2.Size=UDim2.new(0,120,0,14);subLbl2.Position=UDim2.new(0,14,1,-16)
            subLbl2.BackgroundTransparency=1;subLbl2.Text="lagger panel"
            subLbl2.TextColor3=Color3.fromRGB(160,80,255);subLbl2.Font=Enum.Font.GothamBold
            subLbl2.TextSize=9;subLbl2.TextXAlignment=Enum.TextXAlignment.Left;subLbl2.ZIndex=8
            -- Status badge top-right
            guiStatusLabel2=Instance.new("TextLabel");guiStatusLabel2.Name="GuiStatus"
            guiStatusLabel2.Size=UDim2.new(0,42,0,18);guiStatusLabel2.Position=UDim2.new(1,-52,0,13)
            guiStatusLabel2.BackgroundTransparency=0.3;guiStatusLabel2.BackgroundColor3=Color3.fromRGB(20,12,30)
            guiStatusLabel2.Text="OFF";guiStatusLabel2.TextColor3=Color3.fromRGB(255,85,85)
            guiStatusLabel2.TextSize=11;guiStatusLabel2.Font=Enum.Font.GothamBold
            guiStatusLabel2.TextXAlignment=Enum.TextXAlignment.Center;guiStatusLabel2.ZIndex=9;guiStatusLabel2.Parent=hdr2
            Instance.new("UICorner",guiStatusLabel2).CornerRadius=UDim.new(0,5)
            local statusStroke2=Instance.new("UIStroke",guiStatusLabel2);statusStroke2.Color=Color3.fromRGB(130,70,200)
            statusStroke2.Thickness=1.5
            -- Separator (Y 96)
            local line2=Instance.new("Frame");line2.Size=UDim2.new(1,-24,0,1)
            line2.Position=UDim2.new(0,12,0,52);line2.BackgroundColor3=Color3.fromRGB(80,50,120)
            line2.BackgroundTransparency=0.35;line2.Parent=frameThing2
            -- Toggle button (h 34, Y 80)
            btnThing2=Instance.new("TextButton");btnThing2.Size=UDim2.new(1,-24,0,30)
            btnThing2.Position=UDim2.new(0,12,0,60);btnThing2.BackgroundColor3=Color3.fromRGB(25,20,35)
            btnThing2.BorderSizePixel=0;btnThing2.AutoButtonColor=false;btnThing2.Text="";btnThing2.Parent=frameThing2
            Instance.new("UICorner",btnThing2).CornerRadius=UDim.new(0,10)
            local btnBorder2=Instance.new("UIStroke");btnBorder2.Color=Color3.fromRGB(130,70,200)
            btnBorder2.Thickness=2;btnBorder2.ApplyStrokeMode=Enum.ApplyStrokeMode.Border;btnBorder2.Parent=btnThing2
            textThing2=Instance.new("TextLabel");textThing2.Size=UDim2.new(1,-16,1,0)
            textThing2.Position=UDim2.new(0,10,0,0);textThing2.BackgroundTransparency=1
            textThing2.Text="LAGGER";textThing2.TextColor3=Color3.fromRGB(255,255,255)
            textThing2.Font=Enum.Font.GothamBold;textThing2.TextSize=13
            textThing2.TextXAlignment=Enum.TextXAlignment.Left;textThing2.Parent=btnThing2
            -- Logic helpers
            local function getRemote2(road)
                if not road or road=="" then return nil end
                local obj=game;local clean=road:gsub("^game%.","")
                for piece in clean:gmatch("[^%.]+") do if obj then obj=obj[piece] else return nil end end
                return obj
            end
            local function doSpam2(inc,attempts)
                local mainTable={};local spamTable={{}}
                local ptr=spamTable[1]
                for i=1,inc do local nt={};table.insert(ptr,nt);ptr=nt end
                for i=1,15000 do table.insert(mainTable,spamTable);if i%1000==0 then task.wait() end end
                local remoteObj=getRemote2(remoteStuff2)
                if remoteObj then
                    for i=1,attempts do
                        pcall(function()
                            if remoteObj:IsA("RemoteEvent") or remoteObj:IsA("UnreliableRemoteEvent") then remoteObj:FireServer(mainTable)
                            elseif remoteObj:IsA("RemoteFunction") then remoteObj:InvokeServer(mainTable) end
                        end)
                    end
                end
            end
            local function flip2(state)
                active2=state
                if active2 then
                    btnThing2.BackgroundColor3=Color3.fromRGB(45,30,70)
                    btnBorder2.Color=Color3.fromRGB(170,100,255)
                    guiStatusLabel2.Text="ON";guiStatusLabel2.TextColor3=Color3.fromRGB(100,255,130)
                    guiStatusLabel2.BackgroundColor3=Color3.fromRGB(20,40,25)
                    task.spawn(function() while active2 do task.spawn(function() doSpam2(cfg2.x,cfg2.y) end);task.wait(cfg2.z) end end)
                else
                    btnThing2.BackgroundColor3=Color3.fromRGB(25,20,35)
                    btnBorder2.Color=Color3.fromRGB(130,70,200)
                    guiStatusLabel2.Text="OFF";guiStatusLabel2.TextColor3=Color3.fromRGB(255,85,85)
                    guiStatusLabel2.BackgroundColor3=Color3.fromRGB(20,20,30)
                end
            end
            btnThing2.MouseButton1Click:Connect(function() flip2(not active2) end)
            -- Expose toggle for global KB handler
            _G._SilenceLaggerPanelFlip = function() flip2(not active2) end
            _G._SilenceLaggerPanelGui = screenThing2
            -- LeftControl still hides/shows panel
            UIS2.InputBegan:Connect(function(input,processed)
                if processed then return end
                if input.KeyCode==Enum.KeyCode.LeftControl then
                    frameThing2.Visible=not frameThing2.Visible
                end
            end)
        end)
    end
    do
        local row=mkRow(pg,30);mkLabel(row,"Anti Desync")
        local openBtn2=Instance.new("TextButton",row)
        openBtn2.Size=UDim2.new(0,50,0,18);openBtn2.Position=UDim2.new(1,-58,0.5,-9)
        openBtn2.BackgroundColor3=PINK3;openBtn2.BorderSizePixel=0
        openBtn2.Text="Open";openBtn2.TextColor3=WHITE
        openBtn2.Font=Enum.Font.GothamBold;openBtn2.TextSize=9;openBtn2.ZIndex=9
        openBtn2.AutoButtonColor=false
        Instance.new("UICorner",openBtn2).CornerRadius=UDim.new(0,5)
        local _antiDesyncGui=nil
        openBtn2.Activated:Connect(function()
            -- If already open, just bring it to front / do nothing
            if _antiDesyncGui and _antiDesyncGui.Parent then return end
            TweenService:Create(openBtn2,TweenInfo.new(0.08),{BackgroundColor3=Color3.fromRGB(100,50,200)}):Play()
            task.delay(0.15,function() TweenService:Create(openBtn2,TweenInfo.new(0.1),{BackgroundColor3=PINK3}):Play() end)
            openBtn2.Text="Opened"
            task.spawn(function()
                local code=[==[
local Players = game:GetService("Players")
local RunService = game:GetService("RunService")
...
titleLbl.BackgroundTransparency=1; titleLbl.Text="SILENCE AUTO BAT DESYNC"
...
UIS.InputBegan:Connect(function(inp, gp)
    ...
    elseif Keys.autoBatType == "Gamepad" and inp.UserInputType == Enum.UserInputType.Gamepad1 then
        if inp.KeyCode == Keys.autoBat then
            ...
            setToggleVisual(State.autoBatToggled)
                ...
                -- Watch for the gui being closed and reset the button
                _antiDesyncGui = _G._SilenceAntiDesyncGui
                if _antiDesyncGui then
                    task.spawn(function()
                        while _antiDesyncGui and _antiDesyncGui.Parent do
                            task.wait(0.5)
                        end
                        _antiDesyncGui = nil
                        _G._SilenceAntiDesyncGui = nil
                        openBtn2.Text = "Open"
                    end)
                end
            end)
    ...
    mkKBRow(pg,"Anti Desync",KB.AntiDesync,function() saveConfig() end)
    mkSectionLbl(pg,"Danger Zone")
    do
        local row=mkRow(pg,30);mkLabel(row,"Reset Settings")
        local resetBtn=Instance.new("TextButton",row)
        resetBtn.Size=UDim2.new(0,54,0,20);resetBtn.Position=UDim2.new(1,-62,0.5,-10)
    ...
    trackConn(UIS.InputBegan:Connect(function(input,gpe)
        ...
        elseif kbMatch(KB.AntiDesync,kc) then
            local adGui=_G._SilenceAntiDesyncGui
            if adGui and adGui.Parent then
                local m=adGui:FindFirstChild("Main")
...
local function loadConfigKeys()
    if not(isfile and isfile("silencedvs.json")) then return end
    local ok,cfg=pcall(function() return HS:JSONDecode(readfile("silencedvs.json")) end)
    ...
    lk(KB.AutoBat,cfg.autoBatKey);lk(KB.AimbotV2,cfg.aimbotV2Key);lk(KB.LaggerToggle,cfg.laggerToggleKey);lk(KB.TPFloor,cfg.tpFloorKey)

...
local function createLightningEffect(button)
	...
	bolt.ImageColor3 = Color3.fromRGB(255, 255, 255)
	bolt.ZIndex = 21
	...
	glow.BackgroundColor3 = Color3.fromRGB(255, 220, 60)
	...
	for i = 1, 12 do
		...
		spark.BackgroundColor3 = Color3.fromRGB(255, 220, 60)
...
local function globalLightningFlash()
	...
	TweenService:Create(Lighting, TweenInfo.new(0.05), {
		Brightness = 2.5,
		OutdoorAmbient = Color3.fromRGB(255, 255, 200)
	}):Play()
...
local function createMobilePanel()
	...
	task.spawn(function()
		while frame and frame.Parent do
			task.wait(2)
			TweenService:Create(borderStroke, TweenInfo.new(0.2), {Transparency = 0, Color = Color3.fromRGB(255, 255, 100)}):Play()
			task.wait(0.15)
			TweenService:Create(borderStroke, TweenInfo.new(0.3), {Transparency = 0.3, Color = Color3.fromRGB(255, 200, 50)}):Play()
		end
	end)
	...
	local function makeBtn(btn, text, col, row, callback, isToggle)
		...
		btn.MouseLeave:Connect(function()
			if not active then
				TweenService:Create(btn, TweenInfo.new(0.1), {BackgroundColor3 = Color3.fromRGB(18, 18, 28)}):Play()
...
-- ================= INTERFACE GRAPHIQUE =================
local C_BG      = Color3.fromRGB(20, 18, 0)
local C_PANEL   = Color3.fromRGB(25, 22, 2)
local C_ROW     = Color3.fromRGB(22, 20, 2)
local C_ROW_HOV = Color3.fromRGB(40, 35, 5)
local C_BORDER  = Color3.fromRGB(255, 220, 50)
local C_BORDER2 = Color3.fromRGB(255, 240, 70)
local C_HEADER  = Color3.fromRGB(20, 18, 0)
local C_ACCENT  = Color3.fromRGB(255, 245, 100)
local C_ACCENT2 = Color3.fromRGB(255, 220, 60)
local C_DIM     = Color3.fromRGB(200, 180, 50)
local C_WHITE   = Color3.fromRGB(255, 255, 255)
local C_ON_BG   = Color3.fromRGB(255, 220, 60)
local C_OFF_BG  = Color3.fromRGB(40, 35, 10)
local C_KEY_BG  = Color3.fromRGB(30, 25, 5)
...
titleBox.BackgroundColor3 = Color3.fromRGB(20, 18, 0)
titleBox.BorderSizePixel = 0; titleBox.ZIndex = 20; titleBox.Active = true
local tbCorner = Instance.new("UICorner", titleBox); tbCorner.CornerRadius = UDim.new(0, 10)
local tbStroke = Instance.new("UIStroke", titleBox)
tbStroke.Color = Color3.fromRGB(255, 220, 50); tbStroke.Thickness = 1.5; tbStroke.Transparency = 0
...
titleBox.InputBegan:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then _tbDrag=true;_tbDragStart=i.Position;_tbStartPos=titleBox.Position end end)
titleBox.InputChanged:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseMovement or i.UserInputType==Enum.UserInputType.Touch then _tbDragInput=i end end)
UIS.InputChanged:Connect(function(i) if i==_tbDragInput and _tbDrag then local d=i.Position-_tbDragStart; titleBox.Position=UDim2.new(_tbStartPos.X.Scale,_tbStartPos.X.Offset+d.X,_tbStartPos.Y.Scale,_tbStartPos.Y.Offset+d.Y) end end)
UIS.InputEnded:Connect(function(i) if i.UserInputType==Enum.UserInputType.MouseButton1 or i.UserInputType==Enum.UserInputType.Touch then _tbDrag=false end end)
...
Instance.new("UICorner", stealProgressBar).CornerRadius = UDim.new(0, cornerRadius)

local barFill = Instance.new("Frame", stealProgressBar)
barFill.Size = UDim2.new(0, 0, 1, 0)
barFill.BackgroundColor3 = Color3.fromRGB(255, 220, 60)
barFill.BorderSizePixel = 0
Instance.new("UICorner", barFill).CornerRadius = UDim.new(0, cornerRadius)
...
local function loadMainPosition()
	...
	if savedPos and savedPos ~= "" then
		...
		if #parts >= 4 then
			...
			if closeBtn then
				closeBtn.Position = UDim2.new(tonumber(parts[1]), tonumber(parts[2])+310-32, tonumber(parts[3]), tonumber(parts[4])+10)
			end
		end
	end
end
...
closeBtn.BackgroundColor3=Color3.fromRGB(30,25,5); closeBtn.BorderSizePixel=0
closeBtn.Text="X"; closeBtn.TextColor3=Color3.fromRGB(255,220,60)
closeBtn.Font=Enum.Font.GothamBlack; closeBtn.TextSize=13; closeBtn.ZIndex=50
Instance.new("UICorner",closeBtn).CornerRadius=UDim.new(0,6)
local closeBtnStroke = Instance.new("UIStroke",closeBtn)
closeBtnStroke.Color=Color3.fromRGB(255,220,60); closeBtnStroke.Thickness=1.5
...
closeBtn.MouseLeave:Connect(function()
	TweenService:Create(closeBtn,TweenInfo.new(0.12),{BackgroundColor3=Color3.fromRGB(30,25,5),TextColor3=Color3.fromRGB(255,220,60)}):Play()
	TweenService:Create(closeBtnStroke,TweenInfo.new(0.12),{Color=Color3.fromRGB(255,220,60)}):Play()
end)
...
miniBtnStroke.Color = Color3.fromRGB(255,220,60)
...
miniBtn.MouseEnter:Connect(function()
	TweenService:Create(miniBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(40,35,5)}):Play()
end)
miniBtn.MouseLeave:Connect(function()
	TweenService:Create(miniBtn, TweenInfo.new(0.1), {BackgroundColor3=Color3.fromRGB(20,18,0)}):Play()
end)
...
local function makeInputRow(label, default, onChange)
	...
	local rowStroke = Instance.new("UIStroke",row); rowStroke.Color=Color3.fromRGB(255,220,50); rowStroke.Thickness=1
	...
	box.Focused:Connect(function()
		TweenService:Create(bs,TweenInfo.new(0.15),{Color=Color3.fromRGB(255,240,70)}):Play()
		box.BackgroundColor3 = Color3.fromRGB(40,35,5)
	end)
...
local function makeToggleRow(label, defaultKey, defaultOn, onToggle, onKeyChanged)
	...
	if defaultKey then
		...
		keyBtn.TextColor3=Color3.fromRGB(255,255,255); keyBtn.Font=Enum.Font.GothamBold; keyBtn.TextSize=10; keyBtn.ZIndex=5
		Instance.new("UICorner",keyBtn).CornerRadius=UDim.new(0,4)
		local ks=Instance.new("UIStroke",keyBtn); ks.Color=Color3.fromRGB(255,220,50); ks.Thickness=1
		local kListening=false; local kConn
		local function kStop(key)
			kListening=false; if kConn then kConn:Disconnect(); kConn=nil end
			TweenService:Create(ks,TweenInfo.new(0.12),{Color=Color3.fromRGB(255,220,50)}):Play(); keyBtn.TextColor3=Color3.fromRGB(255,255,255)
	...
	local pStroke=Instance.new("UIStroke",pillBg); pStroke.Color=defaultOn and Color3.fromRGB(255,220,60) or C_BORDER; pStroke.Thickness=1.2
	...
	local function setV(on)
		...
		TweenService:Create(pStroke,TweenInfo.new(0.2),{Color=on and Color3.fromRGB(255,220,60) or C_BORDER}):Play()
...
manetteNote.Text = "✓ Compatible manette Xbox / Mobile"
...
local function loadConfig()
	...
	if cfg.grabRadius and type(cfg.grabRadius)=="number" then
		AutoSteal.Radius=cfg.grabRadius; radiusBox.Text=tostring(cfg.grabRadius); if progressRadLbl then progressRadLbl.Text="Radius: "..cfg.grabRadius end end	if cfg.autoStealEnabled then AutoSteal.Enabled=true; setInstaGrab(true); pcall(startAutoSteal) end
...
UIS.InputBegan:Connect(function(inp, gp)
	...
	if inp.UserInputType ~= Enum.UserInputType.Keyboard and inp.UserInputType ~= Enum.UserInputType.Gamepad1 then
		return
	end
